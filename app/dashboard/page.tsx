...
export default async function DashboardPage() {
  ...
  return <main className="min-h-dvh bg-[radial-gradient(circle_at_top_right,_rgba(124,58,237,0.13),_transparent_35%)] px-5 py-6 sm:px-8 lg:px-12">
    <header className="mx-auto flex max-w-7xl items-center justify-between"><a href="/" className="flex items-center gap-2 text-sm font-bold tracking-[0.2em]"><span className="flex size-9 items-center justify-center rounded-xl bg-primary text-primary-foreground">S</span> SNIPLINK</a><div className="flex items-center gap-3"><span className="hidden text-right sm:block"><span className="block text-sm font-semibold">{session.user.username ?? session.user.name}</span><span className="block text-xs text-muted-foreground">{session.user.email}</span></span><span className="flex size-9 items-center justify-center rounded-full bg-primary/10 font-bold text-primary">{initials}</span><LogoutButton /></div></header>
    <section className="mx-auto mt-12 max-w-7xl"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Creator dashboard</p><h1 className="mt-3 text-4xl font-bold tracking-tight">Good to see you, {session.user.name.split(' ')[0]}.</h1><p className="mt-2 text-muted-foreground">Turn every share into measurable momentum.</p></div><a href="#links" className="rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20">View my links</a></div>
      ...
      <div className="mt-8"><PayoutPanel initial={payoutData} /></div>
      <section className="mt-8 rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7"><div className="mb-5"><h2 className="text-xl font-bold">Traffic analytics by country</h2><p className="mt-1 text-sm text-muted-foreground">Verified views and locked publisher rates.</p></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-border text-muted-foreground"><tr><th className="pb-3 font-medium">Country</th><th className="pb-3 font-medium">Clicks</th><th className="pb-3 font-medium">Average CPM</th><th className="pb-3 text-right font-medium">Earnings</th></tr></thead><tbody>{stats?.countryBreakdown?.map((row) => <tr key={row.countryCode} className="border-b border-border last:border-0"><td className="py-3 font-semibold">{row.countryCode}</td><td className="py-3">{row.clicks}</td><td className="py-3">${row.averageCpm.toFixed(2)}</td><td className="py-3 text-right font-semibold">${row.earnings.toFixed(2)}</td></tr>)}</tbody></table>{!stats?.countryBreakdown?.length && <p className="py-6 text-center text-sm text-muted-foreground">Country analytics will appear after verified views.</p>}</div></section>
      <div className="mt-8"><SupportPanel tickets={tickets} /></div>
      <section id="links" className="mt-8 rounded-3xl border border-border bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border p-5 sm:p-7"><div><h2 className="text-xl font-bold">Your links</h2><p className="mt-1 text-sm text-muted-foreground">Every verified view contributes to your estimate.</p></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-semibold">{links.length} total</span></div>{links.length ? <div className="divide-y divide-border">{links.map((link) => <div key={link.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-7"><div className="min-w-0"><p className="font-semibold text-primary">/{link.slug}</p><p className="mt-1 max-w-xl truncate text-sm text-muted-foreground">Private destination</p><p className="mt-2 text-xs text-muted-foreground">Created {link.createdAt.toLocaleDateString()}</p></div><div className="flex items-center gap-3"><span className="rounded-xl bg-secondary px-3 py-2 text-sm font-semibold"><Eye className="mr-1 inline size-4 text-primary" />{link.clicks}</span><a className="inline-flex h-9 items-center gap-2 rounded-md border border-border bg-background px-3 text-sm font-medium hover:bg-secondary" href={`/${link.slug}`} target="_blank" rel="noreferrer"><ExternalLink className="size-3.5" /> Open</a></div></div>)}</div> : <div className="p-10 text-center text-sm text-muted-foreground">Your generated links will appear here.</div>}</section>
    </section>
  </main>
}
