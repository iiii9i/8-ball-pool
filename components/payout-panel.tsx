...
export function PayoutPanel({ initial }: { initial: { balance: number; methods: { method: string; accountDetails: string }[]; requests: Request[] } }) {
  ...
  return <section className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
    ...
    <p className="mt-3 rounded-2xl border border-primary/15 bg-primary/5 px-4 py-3 text-sm text-muted-foreground">Payouts are processed weekly every Sunday for verified accounts. Minimum withdrawal is $10.</p><div className="mt-6 grid gap-4 md:grid-cols-3"><select aria-label="Payment method" value={method} onChange={(e) => setMethod(e.target.value)} className="h-11 rounded-xl border border-border bg-background px-3">{methods.map((item) => <option key={item}>{item}</option>)}</select><input aria-label="Account number" value={details} onChange={(e) => setDetails(e.target.value)} className="h-11 rounded-xl border border-border bg-background px-3" placeholder="Account number / wallet" /><input aria-label="Withdrawal amount" value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="10" step="0.01" className="h-11 rounded-xl border border-border bg-background px-3" placeholder="Amount, minimum $5" /></div>
    <div className="mt-4 flex flex-wrap gap-3"><Button onClick={submitMethod} disabled={pending || !details}>Save payout details</Button><Button onClick={submitWithdrawal} disabled={pending || Number(amount) < 10 || !details} variant="outline">Request withdrawal</Button></div>
