...
type OwnerUser = { id: string; name: string; username: string | null; email: string; role: string; balance: string; status: string; createdAt: Date }
...
export function OwnerConsole({ initialUsers, currentRate }: { initialUsers: OwnerUser[]; currentRate: number }) {
  ...
  async function unlock() {
    ...
    if (!result.ok) setError('That owner key was not accepted.')
      ...
      const [loadedUsers, loadedPayouts] = await Promise.all([getAllUsers(key), getOwnerPayoutData(key)])
